...
def solve_gauss(r_1: List[float], r_2: List[float], r_3: List[float], mu: float, t: float) -> Tuple[
    float, float, float, float, float, float, float]:
    """
    ...
    n_ = [np.cos(W), np.sin(W), 0]
    m_ = [-np.sin(W) * np.cos(i), np.cos(W) * np.cos(i), np.sin(i)]

    a = [[util.dot(n_, r_1 - r_2), util.dot(m_, r_1 - r_2)],
         [util.dot(n_, r_1 - r_3), util.dot(m_, r_1 - r_3)]]
    b = [r2 - r1, r3 - r1]
